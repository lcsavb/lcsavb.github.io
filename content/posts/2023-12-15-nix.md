...
The total container size is 546MB. But with the PDFTK it goes up to 1.5GB. I tried a different approach and modified the base image again to contain a minimal JRE environment:
...
RUN mkdir /opt/app
ADD https://gitlab.com/api/v4/projects/5024297/packages/generic/pdftk-java/v3.3.3/pdftk-all.jar /opt/app/pdftk-all.jar
RUN chmod +x /opt/app/pdftk-all.jar
...
COPY --from=builder /opt/app/pdftk-all.jar /opt/app/
ENV PATH="${JAVA_HOME}/bin:${PATH}"

ENTRYPOINT ["/python3"]
```

The total size of the container dropped to 832MB. There are [other pre-built binaries](https://gitlab.com/pdftk-java/pdftk#pre-built-binaries) which could reduce the image size even further, yet they are experimental.
